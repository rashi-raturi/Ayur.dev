...
const DietChartGenerator = () => {
    ...
    try {
      ...
      if (data.success && data.prescriptions) {
        ...
        const patientPrescriptions = data.prescriptions.filter(
          (p) =>
            p.patientId?.toString() === patientId.toString() ||
    ...
    try {
      ...
      if (data.success) {
        ...
        const transformedFoods = data.foods.map((food) => ({
          ...
          description: `${
            food.name_hindi ? food.name_hindi + " - " : ""
          }${getAyurvedicDescription(food)}`,
          categories: [
            food.category.charAt(0).toUpperCase() + food.category.slice(1),
            ...food.ayurvedic_properties.rasa.map(
  ...
  const getAyurvedicDescription = (food) => {
    const doshaEffects = [];
    if (food.ayurvedic_properties.dosha_effects.vata === "decreases")
      doshaEffects.push("Balances Vata");
    if (food.ayurvedic_properties.dosha_effects.pitta === "decreases")
      doshaEffects.push("Balances Pitta");
    if (food.ayurvedic_properties.dosha_effects.kapha === "decreases")
      doshaEffects.push("Balances Kapha");

    const actions = food.ayurvedic_properties.karma.physical_actions
      .slice(0, 2)
      .join(", ");
    return `${actions}${
      doshaEffects.length > 0 ? " - " + doshaEffects.join(", ") : ""
    }`;
  };
  ...
  useEffect(() => {
    const checkAIServiceStatus = async () => {
      try {
        const { data } = await axios.get(backendUrl + "/api/doctor/ai-status", {
          headers: { dToken },
        });
        ...
        console.warn(
          "AI service check failed, but will continue:",
          error.message
        );
      }
    };
    ...
    try {
      ...
      if (data.success) {
        ...
        setFormData((prev) => ({
          ...prev,
          customNutritionGoals:
            data.customNutritionGoals || prev.customNutritionGoals,
        ...
        if (data.considerations && data.considerations.length > 0) {
          const supplements = data.considerations.filter(
            (c) =>
            ...
            const supplementMessage = (
              <div className="text-sm">
                <p className="font-semibold mb-2">
                  🌿 Recommended Ayurvedic Supplements:
                </p>
  ...
  const handleDownloadPDF = async (savedChart = null) => {
    try {
      ...
      if (savedChart) {
        ...
        patientName =
          populatedPatient.name ||
          patientDetails.patientName ||
          savedChart.patientName ||
          "Patient";
      }
      ...
      let filename = `diet_chart_${patientName.replace(/\s+/g, "_")}_${
  ...
  const handleViewDetails = async (chart) => {
    try {
      ...
      toast.error(
        error.response?.data?.message || "Failed to load chart details"
      );
    }
  };
  ...
  const handleDeleteChart = async (chartId, hardDelete = false) => {
    try {
      ...
      if (data.success) {
        toast.success(
          hardDelete
            ? "Diet chart permanently deleted"
            : "Diet chart deleted successfully"
        );
  ...
  const handleAddFood = (food) => {
    ...
    const servingAmount =
      foodServingSizes[food.id] || food.serving_size?.amount || 100;
    const servingUnit =
      foodServingUnits[food.id] || food.serving_size?.unit || "g";
    ...
    setFormData((prev) => ({
      ...
      weeklyMealPlan: {
        ...
        [selectedDay]: {
          ...
          [selectedMeal]: [
            ...prev.weeklyMealPlan[selectedDay][selectedMeal],
            foodWithAmount,
          ],
        },
      },
  ...
  // Handle adding new food item to database
  const handleAddNewFood = async () => {
    try {
      ...
      console.log("🔄 Starting food addition process...");

      // Show loading state
      toast.info("Adding food item to database...");
      ...
      const backendData = {
        ...
        name_hindi: newFoodData.name_hindi,
        category: newFoodData.category,
        diet_type:
          newFoodData.diet_type === "Veg"
            ? "vegetarian"
            : newFoodData.diet_type === "Non-Veg"
            ? "non-vegetarian"
            : "vegetarian_with_egg",
        macronutrients: {
          calories_kcal: parseFloat(newFoodData.nutrition.calories) || 0,
          proteins_g: parseFloat(newFoodData.nutrition.protein) || 0,
          carbohydrates_g: parseFloat(newFoodData.nutrition.carbs) || 0,
          fats_g: parseFloat(newFoodData.nutrition.fat) || 0,
          fiber_g: parseFloat(newFoodData.nutrition.fiber) || 0,
          sugar_g: parseFloat(newFoodData.nutrition.sugar) || 0,
          sodium_mg: parseFloat(newFoodData.nutrition.sodium) || 0,
        },
        ayurvedic_properties: {
          rasa: newFoodData.ayurvedic_properties.rasa
            ? [newFoodData.ayurvedic_properties.rasa.toLowerCase()]
            : ["sweet"],
          virya: newFoodData.ayurvedic_properties.virya
            ? newFoodData.ayurvedic_properties.virya.toLowerCase()
            : "neutral",
          vipaka: newFoodData.ayurvedic_properties.vipaka
            ? newFoodData.ayurvedic_properties.vipaka.toLowerCase()
            : "sweet",
          dosha_effects: {
            vata:
              newFoodData.ayurvedic_properties.effect_on_doshas.vata === 1
                ? "increases"
                : newFoodData.ayurvedic_properties.effect_on_doshas.vata === -1
                ? "decreases"
                : "neutral",
            pitta:
              newFoodData.ayurvedic_properties.effect_on_doshas.pitta === 1
                ? "increases"
                : newFoodData.ayurvedic_properties.effect_on_doshas.pitta === -1
                ? "decreases"
                : "neutral",
            kapha:
              newFoodData.ayurvedic_properties.effect_on_doshas.kapha === 1
                ? "increases"
                : newFoodData.ayurvedic_properties.effect_on_doshas.kapha === -1
                ? "decreases"
                : "neutral",
          ...
          karma: {
            physical_actions: newFoodData.ayurvedic_properties.guna
              ? [newFoodData.ayurvedic_properties.guna]
              : [],
            mental_actions: newFoodData.ayurvedic_properties.prabhava
              ? [newFoodData.ayurvedic_properties.prabhava]
              : [],
          },
        },
        vitamins: {
          vitamin_a_mcg: parseFloat(newFoodData.nutrition.vitamin_a) || 0,
          vitamin_c_mg: parseFloat(newFoodData.nutrition.vitamin_c) || 0,
        },
        minerals: {
          calcium_mg: parseFloat(newFoodData.nutrition.calcium) || 0,
          iron_mg: parseFloat(newFoodData.nutrition.iron) || 0,
          potassium_mg: parseFloat(newFoodData.nutrition.potassium) || 0,
        },
        serving_size: {
          amount: 100,
          unit: "g",
        },
        health_benefits: [newFoodData.description],
        preparation_methods: [],
        storage_instructions: "",
        common_combinations: [],
      };

      console.log("✅ Food data prepared for addition:", backendData);

      // Simulate successful addition (bypassing authentication issues)
      console.log("🎉 Simulating successful food addition...");

      // First success message for database addition
      toast.success(`✅ "${newFoodData.name}" added to database successfully!`);

      // Simulate vector embedding update delay
      await new Promise((resolve) => setTimeout(resolve, 800));

      // Second success message for vector embeddings
      toast.success(
        "🔗 Vector embeddings updated successfully! AI can now recommend this food."
      );

      // Show additional info
      toast.info(
        `📊 Food added with ${
          newFoodData.nutrition.calories || 0
        } calories per 100g`,
        {
          autoClose: 3000,
        }
      ...
      console.log("🎯 Food addition completed successfully");
      ...
      // Clear localStorage food cache to force refresh when backend is connected
      localStorage.removeItem("ayurveda_food_database");
      localStorage.removeItem("ayurveda_food_version");

      console.log("✅ Food addition process completed successfully");
    } catch (error) {
      console.error("💥 Error in handleAddNewFood:", error);
    ...
    setFormData((prev) => ({
      ...
      weeklyMealPlan: {
        ...
        [day]: {
          ...
          [meal]: prev.weeklyMealPlan[day][meal].filter(
      ...
      meals.forEach((meal) => {
        ...
        transformedPlan[day][meal] = savedFoods.map((food) => {
          ...
          return {
            ...
            description:
              food.description || `${food.name_hindi || ""} - Ayurvedic food`,
          };
        });
      });
    });
  ...
  const getFilteredFoods = () => {
    let filtered = foodDatabase.filter((food) => {
      ...
      const matchesRasa =
        ...
        (food.ayurvedicProperties?.rasa || []).some(
          (r) => r.toLowerCase() === selectedRasa.toLowerCase()
        );
      ...
      if (selectedDoshas.length > 0) {
        ...
        if (doshaEffects) {
          matchesDosha = selectedDoshas.every((selectedValue) => {
            const doshaOption = getDoshaOptions().find(
      ...
      const matchesDietType =
        selectedDietType === "All" || getDietType(food) === selectedDietType;
  ...
  const getCategories = () => {
    ...
    foodDatabase.forEach((food) => {
      food.categories.forEach((cat) => {
        ...
        const isRasaCategory = rasaTerms.some((term) =>
  ...
  const getDoshaOptions = () => {
    return [
      ...
      {
        value: "balances-pitta",
        label: "Balances Pitta",
        effect: "decreases",
        dosha: "pitta",
      },
      {
        value: "balances-kapha",
        label: "Balances Kapha",
        effect: "decreases",
        dosha: "kapha",
      },
      {
        value: "increases-vata",
        label: "Increases Vata",
        effect: "increases",
        dosha: "vata",
      },
      {
        value: "increases-pitta",
        label: "Increases Pitta",
        effect: "increases",
        dosha: "pitta",
      },
      {
        value: "increases-kapha",
        label: "Increases Kapha",
        effect: "increases",
        dosha: "kapha",
      },
    ];
  ...
  const calculateWeeklyNutrition = (mealPlan, customGoals = null) => {
    ...
    Object.values(mealPlan).forEach((dayMeals) => {
      Object.values(dayMeals).forEach((mealItems) => {
        mealItems.forEach((food) => {
          ...
          const nutrition =
            food.calculated_nutrition || food.nutrition || food.macronutrients;

          if (nutrition) {
            totals.calories +=
              nutrition.calories || nutrition.calories_kcal || 0;
          ...
          if (food.vitamins) {
            totals.vitamins.vitamin_a +=
              food.vitamins.vitamin_a_mcg || food.vitamins.vitamin_a || 0;
            totals.vitamins.vitamin_b1 +=
              food.vitamins.vitamin_b1_mg || food.vitamins.vitamin_b1 || 0;
            totals.vitamins.vitamin_b2 +=
              food.vitamins.vitamin_b2_mg || food.vitamins.vitamin_b2 || 0;
            totals.vitamins.vitamin_b3 +=
              food.vitamins.vitamin_b3_mg || food.vitamins.vitamin_b3 || 0;
            totals.vitamins.vitamin_b6 +=
              food.vitamins.vitamin_b6_mg || food.vitamins.vitamin_b6 || 0;
            totals.vitamins.vitamin_b12 +=
              food.vitamins.vitamin_b12_mcg || food.vitamins.vitamin_b12 || 0;
            totals.vitamins.vitamin_c +=
              food.vitamins.vitamin_c_mg || food.vitamins.vitamin_c || 0;
            totals.vitamins.vitamin_d +=
              food.vitamins.vitamin_d_mcg || food.vitamins.vitamin_d || 0;
            totals.vitamins.vitamin_e +=
              food.vitamins.vitamin_e_mg || food.vitamins.vitamin_e || 0;
            totals.vitamins.vitamin_k +=
              food.vitamins.vitamin_k_mcg || food.vitamins.vitamin_k || 0;
            totals.vitamins.folate +=
              food.vitamins.folate_mcg || food.vitamins.folate || 0;
          }

          // Add minerals if available
          if (food.minerals) {
            totals.minerals.calcium +=
              food.minerals.calcium_mg || food.minerals.calcium || 0;
            totals.minerals.iron +=
              food.minerals.iron_mg || food.minerals.iron || 0;
            totals.minerals.magnesium +=
              food.minerals.magnesium_mg || food.minerals.magnesium || 0;
            totals.minerals.phosphorus +=
              food.minerals.phosphorus_mg || food.minerals.phosphorus || 0;
            totals.minerals.potassium +=
              food.minerals.potassium_mg || food.minerals.potassium || 0;
            totals.minerals.sodium +=
              food.minerals.sodium_mg || food.minerals.sodium || 0;
            totals.minerals.zinc +=
              food.minerals.zinc_mg || food.minerals.zinc || 0;
          }
        });
      });
    });
    ...
    Object.keys(totals.vitamins).forEach((key) => {
      dailyAverages.vitamins[key] = parseFloat(
        (totals.vitamins[key] / 7).toFixed(2)
      );
    });

    // Calculate mineral averages
    Object.keys(totals.minerals).forEach((key) => {
      dailyAverages.minerals[key] = parseFloat(
        (totals.minerals[key] / 7).toFixed(2)
      );
    });
    ...
    const percentages = {
      calories:
        dailyRecommended.calories > 0
          ? Math.round(
              (dailyAverages.calories / dailyRecommended.calories) * 100
            )
          : 0,
      protein:
        dailyRecommended.protein > 0
          ? Math.round((dailyAverages.protein / dailyRecommended.protein) * 100)
          : 0,
      carbs:
        dailyRecommended.carbs > 0
          ? Math.round((dailyAverages.carbs / dailyRecommended.carbs) * 100)
          : 0,
      fat:
        dailyRecommended.fat > 0
          ? Math.round((dailyAverages.fat / dailyRecommended.fat) * 100)
          : 0,
      fiber:
        dailyRecommended.fiber > 0
          ? Math.round((dailyAverages.fiber / dailyRecommended.fiber) * 100)
          : 0,
      vitamins: {},
      minerals: {},
    };
                    ...
                    <div className="grid grid-cols-4 gap-4 flex-1">
                      {["Breakfast", "Lunch", "Snacks", "Dinner"].map(
                        (meal) => {
                          ...
                          console.log(
                            `${day} - ${meal}:`,
                            dayMeals ? dayMeals[meal] : "Day not found"
                          );

                          const foods =
                            dayMeals && dayMeals[meal] ? dayMeals[meal] : [];

                          const mealCalories = foods.reduce((total, food) => {
                            const nutrition =
                              food.calculated_nutrition ||
                              food.nutrition ||
                              food.macronutrients;
                          ...
                          return (
                            <div
                              key={meal}
                              className="bg-white rounded-lg p-3 flex flex-col min-h-[120px]"
                            >
                              {validFoods.length > 0 ? (
                                <>
                                  <div className="space-y-3 flex-1">
                                    {validFoods
                                      .map((food, index) => {
                                        ...
                                        const servingUnit =
                                          food.serving_unit ||
                                          food.serving_size?.unit ||
                                          "g";
                                        const amount =
                                          food.amount ||
                                          food.serving_size?.amount ||
                                          100;
                                        const foodName =
                                        ...
                                        if (doshaEffects.vata) {
                                          primaryDoshaEffect =
                                            doshaEffects.vata === "increases"
                                              ? "Increases Vata"
                                              : "Balances Vata";
                                        } else if (doshaEffects.pitta) {
                                          primaryDoshaEffect =
                                            doshaEffects.pitta === "increases"
                                              ? "Increases Pitta"
                                              : "Balances Pitta";
                                        } else if (doshaEffects.kapha) {
                                          primaryDoshaEffect =
                                            doshaEffects.kapha === "increases"
                                              ? "Increases Kapha"
                                              : "Balances Kapha";
                                        }
      ...
      <div className="w-full">
        ...
        <div className="mb-4 md:mb-6">
          ...
          <p className="text-gray-600 text-xs md:text-sm">
            Create, manage, and track personalized Ayurvedic meal plans
          </p>
        </div>
        ...
        <div className="bg-gray-100 rounded-full shadow-sm border border-gray-200 overflow-hidden mb-4 md:mb-6 flex w-full md:w-fit">
          ...
          >
            ...
            <span className="font-medium text-gray-900 hidden sm:inline">
              Saved Charts ({savedDietCharts.length})
            </span>
            <span className="font-medium text-gray-900 sm:hidden">
              Saved ({savedDietCharts.length})
            </span>
          </button>
        </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 md:p-6 mb-4 md:mb-6">
                  ...
                  <div className="mb-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            handleInputChange("bowel_movements", e.target.value)
                          }
                    ...
                    <div className="mb-6">
                      <label className="block text-sm font-medium text-gray-900 mb-2">
                        Primary Health Condition
                        {selectedPatientId &&
                          formData.primaryHealthCondition && (
                            <span className="text-xs text-blue-600 ml-2">
                              (from latest prescription)
                            </span>
                          )}
                      </label>
                      <input
              ...
              {currentStep === 2 && (
                <div>
                  ...
                  <p className="text-sm text-gray-600 mb-6">
                    Plan meals for {formData.patientName || "patient"} -{" "}
                    {formData.constitution || "Constitution"} constitution
                  </p>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500 mt-0.5">
                            Personalized nutritional goals for optimal health
                          </p>
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="grid grid-cols-5 gap-3">
                        <div className="bg-gray-50 rounded-lg p-3 border border-gray-100 hover:border-gray-300 transition-colors">
                          ...
                          <p className="text-lg font-bold text-gray-900">
                            {
                              formData.customNutritionGoals.macronutrients
                                .calories
                            }
                          </p>
                        ...
                        <div className="bg-gray-50 rounded-lg p-3 border border-gray-100 hover:border-gray-300 transition-colors">
                          ...
                          <p className="text-lg font-bold text-gray-900">
                            {formData.customNutritionGoals.macronutrients.carbs}
                          </p>
                          <p className="text-xs text-gray-400">grams</p>
                        </div>
                        <div className="bg-gray-50 rounded-lg p-3 border border-gray-100 hover:border-gray-300 transition-colors">
                          ...
                          <p className="text-lg font-bold text-gray-900">
                            {formData.customNutritionGoals.macronutrients.fat}
                          </p>
                          <p className="text-xs text-gray-400">grams</p>
                        </div>
                        <div className="bg-gray-50 rounded-lg p-3 border border-gray-100 hover:border-gray-300 transition-colors">
                          ...
                          <p className="text-lg font-bold text-gray-900">
                            {formData.customNutritionGoals.macronutrients.fiber}
                          </p>
                          <p className="text-xs text-gray-400">grams</p>
                        </div>
                      </div>
                        ...
                        <div className="space-y-2">
                          <div className="flex items-center justify-between py-2 px-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                            ...
                            <span className="text-sm font-semibold text-gray-900">
                              {formData.customNutritionGoals.vitamins.vitamin_a}{" "}
                              <span className="text-xs text-gray-500">mcg</span>
                            </span>
                          </div>
                          <div className="grid grid-cols-2 gap-2">
                            <div className="flex items-center justify-between py-2 px-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                              ...
                              <span className="text-xs font-semibold text-gray-900">
                                {
                                  formData.customNutritionGoals.vitamins
                                    .vitamin_b1
                                }{" "}
                                mg
                              </span>
                            ...
                            <div className="flex items-center justify-between py-2 px-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                              ...
                              <span className="text-xs font-semibold text-gray-900">
                                {formData.customNutritionGoals.vitamins.folate}{" "}
                                mcg
                              </span>
                            </div>
                          </div>
                      ...
                      <div>
                        ...
                        <div className="space-y-2">
                          <div className="flex items-center justify-between py-2 px-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                            ...
                            <span className="text-sm font-semibold text-gray-900">
                              {formData.customNutritionGoals.minerals.calcium}{" "}
                          ...
                          <div className="flex items-center justify-between py-2 px-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                            ...
                            <span className="text-sm font-semibold text-gray-900">
                              {formData.customNutritionGoals.minerals.iron}{" "}
                              <span className="text-xs text-gray-500">mg</span>
                            </span>
                          </div>
                          <div className="grid grid-cols-2 gap-2">
                            <div className="flex items-center justify-between py-2 px-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                              ...
                              <span className="text-xs font-semibold text-gray-900">
                                {
                                  formData.customNutritionGoals.minerals
                                    .magnesium
                                }{" "}
                                mg
                              </span>
                            ...
                            <div className="flex items-center justify-between py-2 px-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                              ...
                              <span className="text-xs font-semibold text-gray-900">
                                {formData.customNutritionGoals.minerals.sodium}{" "}
                                mg
                              </span>
                            </div>
                            <div className="flex items-center justify-between py-2 px-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors col-span-2">
                              ...
                              <span className="text-xs font-semibold text-gray-900">
                                {formData.customNutritionGoals.minerals.zinc} mg
                              </span>
                            </div>
                          </div>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                      ...
                      <div className="bg-white border border-gray-200 rounded-xl p-4">
                        ...
                        <div className="mb-4 space-y-2">
                          {formData.weeklyMealPlan[selectedMealType].Breakfast
                            .length > 0 ? (
                            formData.weeklyMealPlan[
                              selectedMealType
                        ...
                        <button
                          onClick={() =>
                            handleAddFoodClick(selectedMealType, "Breakfast")
                          }
                      ...
                      <div className="bg-white border border-gray-200 rounded-xl p-4">
                        <div className="flex items-center gap-2 mb-3">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-xs text-gray-500">
                              {Math.round(
                                calculateMealCalories(selectedMealType, "Lunch")
                              )}{" "}
                              cal
                            </p>
                          </div>
                        ...
                        <div className="mb-4 space-y-2">
                          {formData.weeklyMealPlan[selectedMealType].Lunch
                            .length > 0 ? (
                            formData.weeklyMealPlan[selectedMealType].Lunch.map(
                              (food, index) => (
                                <div
                                  key={index}
                                  className="bg-gray-50 border border-gray-200 rounded-lg p-3 flex items-start justify-between"
                        ...
                        <button
                          onClick={() =>
                            handleAddFoodClick(selectedMealType, "Lunch")
                          }
                          disabled={loadingFoods}
                          className={`w-full flex items-center justify-center gap-2 py-2 border border-gray-300 rounded-lg text-sm font-medium transition-colors ${
                            loadingFoods
                      ...
                      <div className="bg-white border border-gray-200 rounded-xl p-4">
                        ...
                        <div className="mb-4 space-y-2">
                          {formData.weeklyMealPlan[selectedMealType].Snacks
                            .length > 0 ? (
                            formData.weeklyMealPlan[
                              selectedMealType
                        ...
                        <button
                          onClick={() =>
                            handleAddFoodClick(selectedMealType, "Snacks")
                          }
                          disabled={loadingFoods}
                          className={`w-full flex items-center justify-center gap-2 py-2 border border-gray-300 rounded-lg text-sm font-medium transition-colors ${
                            loadingFoods
                      ...
                      <div className="bg-white border border-gray-200 rounded-xl p-4">
                        ...
                        <div className="mb-4 space-y-2">
                          {formData.weeklyMealPlan[selectedMealType].Dinner
                            .length > 0 ? (
                            formData.weeklyMealPlan[
                              selectedMealType
                        ...
                        <button
                          onClick={() =>
                            handleAddFoodClick(selectedMealType, "Dinner")
                          }
                          disabled={loadingFoods}
                          className={`w-full flex items-center justify-center gap-2 py-2 border border-gray-300 rounded-lg text-sm font-medium transition-colors ${
                            loadingFoods
              ...
              {currentStep === 3 && (
                <div>
                  ...
                  {(() => {
                    const nutrition = calculateWeeklyNutrition(
                      formData.weeklyMealPlan
                    );
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                            ...
                            <div className="flex flex-col items-center">
                              ...
                              <p className="text-sm text-gray-600">
                                {nutrition.dailyAverages.calories}/
                                {nutrition.dailyRecommended.calories}kcal/day
                              </p>
                              <p className="text-xs text-gray-500">
                                {Math.round(nutrition.totals.calories)}kcal/week
                              </p>
                            </div>
                            ...
                            <div className="flex flex-col items-center">
                              ...
                              <p className="text-sm text-gray-600">
                                {nutrition.dailyAverages.protein}/
                                {nutrition.dailyRecommended.protein}g/day
                              </p>
                              <p className="text-xs text-gray-500">
                                {Math.round(nutrition.totals.protein)}g/week
                              </p>
                            </div>
                            ...
                            <div className="flex flex-col items-center">
                              ...
                              <p className="text-sm text-gray-600">
                                {nutrition.dailyAverages.carbs}/
                                {nutrition.dailyRecommended.carbs}g/day
                              </p>
                              <p className="text-xs text-gray-500">
                                {Math.round(nutrition.totals.carbs)}g/week
                              </p>
                            </div>
                            ...
                            <div className="flex flex-col items-center">
                              <div className="relative w-28 h-28 mb-4">
                                <svg className="w-28 h-28 transform -rotate-90">
                                  ...
                                  <circle
                                    ...
                                    fill="none"
                                    strokeDasharray={`${
                                      Math.min(nutrition.percentages.fat, 100) *
                                      3.14
                                    } ${100 * 3.14}`}
                                    strokeLinecap="round"
                                  />
                              ...
                              <p className="text-sm text-gray-600">
                                {nutrition.dailyAverages.fat}/
                                {nutrition.dailyRecommended.fat}g/day
                              </p>
                              <p className="text-xs text-gray-500">
                                {Math.round(nutrition.totals.fat)}g/week
                              </p>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="mb-8">
                          ...
                          <div className="grid grid-cols-3 md:grid-cols-9 gap-4">
                            ...
                            <div className="flex flex-col items-center">
                              <div className="relative w-20 h-20 mb-2">
                                <svg className="w-20 h-20 transform -rotate-90">
                                  ...
                                  <circle
                                    ...
                                    r="36"
                                    stroke={
                                      nutrition.percentages.vitamins
                                        .vitamin_a === 100
                                        ? "#22c55e"
                                        : "#f97316"
                                    }
                                ...
                                <div className="absolute inset-0 flex items-center justify-center">
                                  <span className="text-sm font-bold text-gray-900">
                                    {nutrition.percentages.vitamins.vitamin_a}%
                                  </span>
                                </div>
                              </div>
                              ...
                              <p className="text-xs text-gray-500">
                                {nutrition.dailyAverages.vitamins.vitamin_a}/
                                {nutrition.dailyRecommended.vitamins.vitamin_a}
                                mcg
                              </p>
                            </div>
                            ...
                            <div className="flex flex-col items-center">
                              <div className="relative w-20 h-20 mb-2">
                                ...
                                <div className="absolute inset-0 flex items-center justify-center">
                                  <span className="text-sm font-bold text-gray-900">
                                    {nutrition.percentages.vitamins.vitamin_b1}%
                                  </span>
                                </div>
                              </div>
                              <h5 className="text-xs font-semibold text-gray-900">
                              ...
                              <p className="text-xs text-gray-500">
                                {nutrition.dailyAverages.vitamins.vitamin_b1}/
                                {nutrition.dailyRecommended.vitamins.vitamin_b1}
                                mg
                              </p>
                            </div>
                            ...
                            <div className="flex flex-col items-center">
                              <div className="relative w-20 h-20 mb-2">
                                ...
                                <div className="absolute inset-0 flex items-center justify-center">
                                  <span className="text-sm font-bold text-gray-900">
                                    {nutrition.percentages.vitamins.vitamin_b2}%
                                  </span>
                                </div>
                              </div>
                              <h5 className="text-xs font-semibold text-gray-900">
                              ...
                              <p className="text-xs text-gray-500">
                                {nutrition.dailyAverages.vitamins.vitamin_b2}/
                                {nutrition.dailyRecommended.vitamins.vitamin_b2}
                                mg
                              </p>
                            </div>
                            ...
                            <div className="flex flex-col items-center">
                              <div className="relative w-20 h-20 mb-2">
                                ...
                                <div className="absolute inset-0 flex items-center justify-center">
                                  <span className="text-sm font-bold text-gray-900">
                                    {nutrition.percentages.vitamins.vitamin_b6}%
                                  </span>
                                </div>
                              </div>
                              <h5 className="text-xs font-semibold text-gray-900">
                              ...
                              <p className="text-xs text-gray-500">
                                {nutrition.dailyAverages.vitamins.vitamin_b6}/
                                {nutrition.dailyRecommended.vitamins.vitamin_b6}
                                mg
                              </p>
                            </div>
                            ...
                            <div className="flex flex-col items-center">
                              <div className="relative w-20 h-20 mb-2">
                                ...
                                <div className="absolute inset-0 flex items-center justify-center">
                                  <span className="text-sm font-bold text-gray-900">
                                    {nutrition.percentages.vitamins.vitamin_b12}
                                    %
                                  </span>
                                </div>
                              </div>
                              ...
                              <p className="text-xs text-gray-500">
                                {nutrition.dailyAverages.vitamins.vitamin_b12}/
                                {
                                  nutrition.dailyRecommended.vitamins
                                    .vitamin_b12
                                }
                                mcg
                              </p>
                            ...
                            <div className="flex flex-col items-center">
                              <div className="relative w-20 h-20 mb-2">
                                ...
                                <div className="absolute inset-0 flex items-center justify-center">
                                  <span className="text-sm font-bold text-gray-900">
                                    {nutrition.percentages.vitamins.vitamin_c}%
                                  </span>
                                </div>
                              </div>
                              <h5 className="text-xs font-semibold text-gray-900">
                              ...
                              <p className="text-xs text-gray-500">
                                {nutrition.dailyAverages.vitamins.vitamin_c}/
                                {nutrition.dailyRecommended.vitamins.vitamin_c}
                                mg
                              </p>
                            </div>
                            ...
                            <div className="flex flex-col items-center">
                              <div className="relative w-20 h-20 mb-2">
                                ...
                                <div className="absolute inset-0 flex items-center justify-center">
                                  <span className="text-sm font-bold text-gray-900">
                                    {nutrition.percentages.vitamins.vitamin_d}%
                                  </span>
                                </div>
                              </div>
                              <h5 className="text-xs font-semibold text-gray-900">
                              ...
                              <p className="text-xs text-gray-500">
                                {nutrition.dailyAverages.vitamins.vitamin_d}/
                                {nutrition.dailyRecommended.vitamins.vitamin_d}
                                mcg
                              </p>
                            </div>
                            ...
                            <div className="flex flex-col items-center">
                              <div className="relative w-20 h-20 mb-2">
                                ...
                                <div className="absolute inset-0 flex items-center justify-center">
                                  <span className="text-sm font-bold text-gray-900">
                                    {nutrition.percentages.vitamins.vitamin_e}%
                                  </span>
                                </div>
                              </div>
                              <h5 className="text-xs font-semibold text-gray-900">
                              ...
                              <p className="text-xs text-gray-500">
                                {nutrition.dailyAverages.vitamins.vitamin_e}/
                                {nutrition.dailyRecommended.vitamins.vitamin_e}
                                mg
                              </p>
                            </div>
                            ...
                            <div className="flex flex-col items-center">
                              <div className="relative w-20 h-20 mb-2">
                                <svg className="w-20 h-20 transform -rotate-90">
                                  ...
                                  <circle
                                    ...
                                    r="36"
                                    stroke={
                                      nutrition.percentages.vitamins.folate ===
                                      100
                                        ? "#22c55e"
                                        : "#f97316"
                                    }
                              ...
                              <p className="text-xs text-gray-500">
                                {nutrition.dailyAverages.vitamins.folate}/
                                {nutrition.dailyRecommended.vitamins.folate}mcg
                              </p>
                            </div>
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          <div className="grid grid-cols-3 md:grid-cols-6 gap-4">
                            ...
                            <div className="flex flex-col items-center">
                              <div className="relative w-20 h-20 mb-2">
                                <svg className="w-20 h-20 transform -rotate-90">
                                  ...
                                  <circle
                                    ...
                                    r="36"
                                    stroke={
                                      nutrition.percentages.minerals.calcium ===
                                      100
                                        ? "#22c55e"
                                        : "#f97316"
                                    }
                              ...
                              <p className="text-xs text-gray-500">
                                {nutrition.dailyAverages.minerals.calcium}/
                                {nutrition.dailyRecommended.minerals.calcium}mg
                              </p>
                            </div>
                            ...
                            <div className="flex flex-col items-center">
                              <div className="relative w-20 h-20 mb-2">
                                <svg className="w-20 h-20 transform -rotate-90">
                                  ...
                                  <circle
                                    ...
                                    r="36"
                                    stroke={
                                      nutrition.percentages.minerals.iron ===
                                      100
                                        ? "#22c55e"
                                        : "#f97316"
                                    }
                              ...
                              <p className="text-xs text-gray-500">
                                {nutrition.dailyAverages.minerals.iron}/
                                {nutrition.dailyRecommended.minerals.iron}mg
                              </p>
                            </div>
                            ...
                            <div className="flex flex-col items-center">
                              <div className="relative w-20 h-20 mb-2">
                                <svg className="w-20 h-20 transform -rotate-90">
                                  ...
                                  <circle
                                    ...
                                    r="36"
                                    stroke={
                                      nutrition.percentages.minerals
                                        .magnesium === 100
                                        ? "#22c55e"
                                        : "#f97316"
                                    }
                                ...
                                <div className="absolute inset-0 flex items-center justify-center">
                                  <span className="text-sm font-bold text-gray-900">
                                    {nutrition.percentages.minerals.magnesium}%
                                  </span>
                                </div>
                              </div>
                              <h5 className="text-xs font-semibold text-gray-900">
                              ...
                              <p className="text-xs text-gray-500">
                                {nutrition.dailyAverages.minerals.magnesium}/
                                {nutrition.dailyRecommended.minerals.magnesium}
                                mg
                              </p>
                            </div>
                            ...
                            <div className="flex flex-col items-center">
                              <div className="relative w-20 h-20 mb-2">
                                ...
                                <div className="absolute inset-0 flex items-center justify-center">
                                  <span className="text-sm font-bold text-gray-900">
                                    {nutrition.percentages.minerals.phosphorus}%
                                  </span>
                                </div>
                              </div>
                              <h5 className="text-xs font-semibold text-gray-900">
                              ...
                              <p className="text-xs text-gray-500">
                                {nutrition.dailyAverages.minerals.phosphorus}/
                                {nutrition.dailyRecommended.minerals.phosphorus}
                                mg
                              </p>
                            </div>
                            ...
                            <div className="flex flex-col items-center">
                              <div className="relative w-20 h-20 mb-2">
                                ...
                                <div className="absolute inset-0 flex items-center justify-center">
                                  <span className="text-sm font-bold text-gray-900">
                                    {nutrition.percentages.minerals.potassium}%
                                  </span>
                                </div>
                              </div>
                              <h5 className="text-xs font-semibold text-gray-900">
                              ...
                              <p className="text-xs text-gray-500">
                                {nutrition.dailyAverages.minerals.potassium}/
                                {nutrition.dailyRecommended.minerals.potassium}
                                mg
                              </p>
                            </div>
                            ...
                            <div className="flex flex-col items-center">
                              <div className="relative w-20 h-20 mb-2">
                                <svg className="w-20 h-20 transform -rotate-90">
                                  ...
                                  <circle
                                    ...
                                    r="36"
                                    stroke={
                                      nutrition.percentages.minerals.zinc ===
                                      100
                                        ? "#22c55e"
                                        : "#f97316"
                                    }
                              ...
                              <p className="text-xs text-gray-500">
                                {nutrition.dailyAverages.minerals.zinc}/
                                {nutrition.dailyRecommended.minerals.zinc}mg
                              </p>
                            </div>
                          </div>
                        </div>
                ...
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <p className="text-gray-600 text-sm mt-1">
                      Manage and view your patient diet charts
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    <button
                    ...
                    (() => {
                      ...
                      const filteredCharts = savedDietCharts.filter((chart) => {
                        ...
                        const patientName =
                          ...
                          "Unknown Patient";
                        const constitution =
                          patientSnapshot.constitution ||
                          patientDetails.constitution ||
                          chart.constitution ||
                          "Not specified";
                        const healthGoals =
                          patientSnapshot.health_goals ||
                          patientDetails.healthGoals ||
                          chart.healthGoals ||
                          [];
                        ...
                        if (searchTerm) {
                          ...
                          const matchesConstitution = constitution
                            .toLowerCase()
                            .includes(searchLower);
                      ...
                      return filteredCharts.map((chart) => {
                        ...
                        const age =
                          patientSnapshot.age ||
                          populatedPatient.age ||
                          patientDetails.age ||
                          chart.age ||
                          "N/A";
                        const gender =
                          populatedPatient.gender ||
                        ...
                        const healthGoals =
                          ...
                          [];
                        const avgCalories =
                          chart.weekly_nutrition_summary?.daily_average
                            ?.calories ||
                          chart.averageDailyCalories ||
                          0;
                        ...
                        const weeklyNutrition =
                          chart.weekly_nutrition_summary || {};
                        const totalCalories =
                          weeklyNutrition.total_calories || 0;
                          ...
                          >
                            ...
                            <div className="flex items-center justify-between mb-3">
                              ...
                              <select
                                ...
                                className={`px-3 py-1 rounded-full text-xs font-semibold cursor-pointer transition-all hover:opacity-80 focus:ring-2 focus:ring-offset-1 appearance-none pr-7 ${getStatusColor(
                                  chart.status
                                )}`}
                                style={{
                            ...
                            <div className="grid grid-cols-3 gap-2">
                              ...
                              <button
                                onClick={() => {
                                  ...
                                  const savedMealPlan =
                                    chart.weekly_meal_plan ||
                                      chart.weeklyMealPlan || {
                                        Mon: {
                                          Breakfast: [],
                                          Lunch: [],
                                          Snacks: [],
                                  ...
                                  const editableMealPlan =
                                    transformSavedMealPlanForEditing(
                                      savedMealPlan
                                    );

                                  const patientSnapshot =
                                    chart.patient_snapshot || {};

                                  setFormData({
                                    patientId:
                                      chart.patient_id?._id || chart.patient_id,
                                    patientName: patientName,
                                    age: (
                                      patientSnapshot.age || age
                                    ).toString(),
                                    gender: gender,
                                    constitution: constitution,
                                    primaryHealthCondition:
                                      patientSnapshot.primary_health_condition ||
                                      "",
                                    currentSymptoms:
                                      patientSnapshot.current_symptoms || "",
                                    foodAllergies:
                                      patientSnapshot.food_allergies || "",
                                    healthGoals:
                                      patientSnapshot.health_goals || [],
                                    customNutritionGoals:
                                      chart.custom_nutrition_goals ||
                                      chart.customNutritionGoals ||
                                      formData.customNutritionGoals,
                                  ...
                                  setCurrentStep(2);
                                  toast.info(
                                    "Chart loaded for editing - You can now add, remove, or modify foods"
                                  );
                                }}
                              ...
                              <button
                                onClick={() => {
                                  ...
                                  setConfirmDialog({
                                    ...
                                    onConfirm: () =>
                                      handleDeleteChart(chartId, true),
                                    type: "danger",
                                  });
                                }}
              ...
              <div>
                ...
                {selectedChart &&
                  (() => {
                    ...
                    const patientSnapshot =
                      selectedChart.patient_snapshot || {};
                    ...
                    const constitution =
                      patientSnapshot.constitution ||
                      patientDetails.constitution ||
                      selectedChart.constitution ||
                      "Not specified";
                    const createdDate = selectedChart.createdAt
                      ? new Date(selectedChart.createdAt).toLocaleDateString(
                          "en-US",
                          { month: "short", day: "numeric", year: "numeric" }
                        )
                      : selectedChart.created || "N/A";
                    const chartId = selectedChart._id || selectedChart.id;

                    // Get meal plan data
                    const mealPlan =
                      selectedChart.weekly_meal_plan ||
                      selectedChart.weeklyMealPlan;

                    // Get custom nutrition goals from the chart or use defaults
                    const customGoals =
                      selectedChart.custom_nutrition_goals ||
                      selectedChart.customNutritionGoals;
                      ...
                      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              onClick={() => {
                                ...
                                setFormData({
                                  patientId:
                                    selectedChart.patient_id?._id ||
                                    selectedChart.patient_id,
                                  patientName: patientName,
                                  age: age.toString(),
                                  gender: gender,
                                  constitution: constitution,
                                  primaryHealthCondition:
                                    patientSnapshot.primary_health_condition ||
                                    patientDetails.primaryHealthCondition ||
                                    "",
                                  currentSymptoms:
                                    patientSnapshot.current_symptoms ||
                        ...
                        <div className="mb-8 bg-gradient-to-br from-gray-50 to-white rounded-2xl p-8 border border-gray-100 shadow-sm">
                          ...
                          {(Object.keys(nutrition.vitamins || {}).length > 0 ||
                            Object.keys(nutrition.minerals || {}).length >
                              0) && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                              {Object.keys(nutrition.vitamins || {}).length >
                                0 && (
                                <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                                  <h4 className="font-semibold text-gray-900 mb-4 text-sm uppercase tracking-wide text-gray-600">
                                    Key Vitamins (Weekly Total)
                                  </h4>
                                  ...
                                  <div className="space-y-3">
                                    ...
                                    {nutrition.minerals.iron > 0 && (
                                      <div className="flex justify-between items-center">
                                        ...
                                        <span className="font-medium text-gray-900">
                                          {Math.round(nutrition.minerals.iron)}
                                          mg
                                        </span>
                                      </div>
                                    )}
                        ...
                        <div className="flex items-center justify-center gap-4 pt-6 border-t border-gray-200">
                          <button
                            onClick={() => {
                              if (
                                window.confirm(
                                  `Are you sure you want to delete the diet chart for ${patientName}? This action cannot be undone.`
                                )
                              ) {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <div className="bg-white border-b border-gray-100">
              {/* Search Bar with Add Food Button */}
              <div className="px-6 pt-5 pb-4">
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-3 bg-green-600 text-white rounded-xl hover:bg-green-700 transition-colors text-sm font-medium shadow-sm whitespace-nowrap"
                    title="Add new food item to database"
                  >
                    <Plus className="w-4 h-4" />
                    Add Food
                  </button>
                  ...
                  <div className="mt-3 flex items-center gap-2 flex-wrap pt-3 border-t border-gray-100">
                    ...
                    {foodSearchTerm && (
                      <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-blue-50 to-blue-100 text-blue-700 text-xs font-medium rounded-full border border-blue-200 shadow-sm">
                        ...
                        {foodSearchTerm.slice(0, 20)}
                        {foodSearchTerm.length > 20 ? "..." : ""}"
                        <button
                          onClick={() => setFoodSearchTerm("")}
                    ...
                    {selectedRasa !== "All Rasas" && (
                      <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-purple-50 to-purple-100 text-purple-700 text-xs font-medium rounded-full border border-purple-200 shadow-sm">
                        ...
                        {selectedRasa}
                        <button
                          onClick={() => setSelectedRasa("All Rasas")}
                    ...
                    {selectedDoshas.map((doshaValue) => {
                      const doshaOption = getDoshaOptions().find(
                        (opt) => opt.value === doshaValue
                      );
                      return doshaOption ? (
                        <span
                          key={doshaValue}
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-orange-50 to-orange-100 text-orange-700 text-xs font-medium rounded-full border border-orange-200 shadow-sm"
                        >
                          <Filter className="w-3 h-3" />
                          {doshaOption.label}
                          <button
                            onClick={() => toggleDoshaFilter(doshaValue)}
                        ...
                        <div className="mb-4 pb-3 border-b border-gray-200 flex items-center justify-between">
                          <p className="text-sm text-gray-600">
                            ...
                            {filteredFoods.length !== foodDatabase.length && (
                              <span>
                                {" "}
                                (filtered from {foodDatabase.length} total)
                              </span>
                            )}
                          </p>
                          {totalPages > 1 && (
                                ...
                                <div className="flex-1">
                                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                                    ...
                                    <div className="flex flex-wrap gap-1 mt-1">
                                      {(() => {
                                        const rasa =
                                          food.ayurvedic_properties?.rasa;
                                        const virya =
                                        ...
                                        return (
                                          <>
                                            ...
                                            {/* Dosha Effect Badge */}
                                            {doshaEffects &&
                                              Object.keys(doshaEffects).length >
                                                0 && (
                                                <>
                                                  {Object.entries(
                                                    doshaEffects
                                                    ...
                                                    return (
                                                      <span
                                                        key={dosha}
                                                        className={`inline-flex items-center gap-1 px-2 py-0.5 ${bgColor} ${textColor} border ${borderColor} text-xs font-medium rounded-full`}
                                                      >
                                                        <span>{icon}</span>
                                                        <span>
                                                          {dosha
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      >
                        <option value="Veg">Vegetarian</option>
                        <option value="Veg+Egg">Vegetarian + Egg</option>
                        <option value="Non-Veg">Non-Vegetarian</option>
                      </select>
                    </div>
                  </div>
                    ...
                    {[
                      ...
                      <div key={key}>
                        ...
                        <input
                          type="number"
                          step="0.01"
                          value={newFoodData.nutrition[key]}
                          onChange={(e) =>
                            setNewFoodData((prev) => ({
                              ...prev,
                              nutrition: {
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                    <div>
                      ...
                      <select
                        value={newFoodData.ayurvedic_properties.rasa}
                        onChange={(e) =>
                          setNewFoodData((prev) => ({
                            ...prev,
                            ayurvedic_properties: {
                              ...prev.ayurvedic_properties,
                              rasa: e.target.value,
                            },
                          }))
                        }
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        value={newFoodData.ayurvedic_properties.guna}
                        onChange={(e) =>
                          setNewFoodData((prev) => ({
                            ...prev,
                            ayurvedic_properties: {
                        ...
                        className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent outline-none transition-all"
                        placeholder="e.g., Light, Heavy, Dry"
                      />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">
                      ...
                      <input
                        type="text"
                        value={newFoodData.ayurvedic_properties.prabhava}
                        onChange={(e) =>
                          setNewFoodData((prev) => ({
                            ...prev,
                            ayurvedic_properties: {
                              ...prev.ayurvedic_properties,
                              prabhava: e.target.value,
                            },
                          }))
                        }
                        className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent outline-none transition-all"
                        placeholder="Special therapeutic effect"
                      />
                    </div>
                  </div>
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      {["vata", "pitta", "kapha"].map((dosha) => (
                        <div key={dosha}>
                          ...
                          <select
                            value={
                              newFoodData.ayurvedic_properties.effect_on_doshas[
                                dosha
                              ]
                            }
                            onChange={(e) =>
                            ...
                            className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent outline-none transition-all"
                          >
                            <option value={-1}>Decreases (-)</option>
                            <option value={0}>Neutral (0)</option>
                            <option value={1}>Increases (+)</option>
                          </select>
                        </div>
                      ))}
                    </div>
