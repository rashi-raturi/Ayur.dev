...
const PatientProfileModal = ({ isOpen, onClose, patientId }) => {
    ...
    const downloadSummaryAsPDF = async () => {
        ...
        try {
            ...
            yPosition = addText('PATIENT MEDICAL SUMMARY', margin, 15, pageWidth - 2 * margin, 18, 'bold')
            yPosition = addText('Generated by Ayurveda Healthcare System', margin, 25, pageWidth - 2 * margin, 12)
            ...
            yPosition = addText(`Name: ${patientData.name}`, margin, yPosition, pageWidth - 2 * margin, 11)
            yPosition = addText(`Email: ${patientData.email}`, margin, yPosition, pageWidth - 2 * margin, 11)
            yPosition = addText(`Phone: ${patientData.phone || 'Not provided'}`, margin, yPosition, pageWidth - 2 * margin, 11)
            
            if (patientData.dob) {
                const age = calculateAge(patientData.dob)
                yPosition = addText(`Age: ${!isNaN(age) && age > 0 ? `${age} years` : 'Not provided'}`, margin, yPosition, pageWidth - 2 * margin, 11)
            }
            
            yPosition = addText(`Gender: ${patientData.gender || 'Not specified'}`, margin, yPosition, pageWidth - 2 * margin, 11)
            yPosition = addText(`Date of Birth: ${patientData.dob ? new Date(patientData.dob).toLocaleDateString() : 'Not provided'}`, margin, yPosition, pageWidth - 2 * margin, 11)
            
            if (patientData.address?.line1) {
                yPosition = addText(`Address: ${patientData.address.line1}${patientData.address.line2 ? ', ' + patientData.address.line2 : ''}`, margin, yPosition, pageWidth - 2 * margin, 11)
            }
            
            yPosition += 10

            // Summary Date
            yPosition = addText(`Report Generated: ${new Date().toLocaleDateString('en-US', { 
                year: 'numeric', 
                month: 'long', 
                day: 'numeric',
                hour: '2-digit',
                minute: '2-digit'
            })}`, margin, yPosition, pageWidth - 2 * margin, 10, 'italic')
            yPosition += 15
            ...
            if (typeof summary === 'object') {
                ...
                if (summary.summary) {
                    yPosition = addText('MEDICAL SUMMARY', margin, yPosition, pageWidth - 2 * margin, 14, 'bold')
                    yPosition += 5
                    ...
                    if (typeof summary.summary === 'string') {
                        ...
                        for (let line of summaryLines) {
                            if (line.trim()) {
                                yPosition = addText(`• ${line.trim()}`, margin + 5, yPosition, pageWidth - 2 * margin - 5, 10)
                                yPosition += 2
                            }
                        }
                    } else if (Array.isArray(summary.summary)) {
                        for (let item of summary.summary) {
                            yPosition = addText(`• ${typeof item === 'object' ? JSON.stringify(item) : item}`, margin + 5, yPosition, pageWidth - 2 * margin - 5, 10)
                            yPosition += 2
                        }
                    } else if (typeof summary.summary === 'object') {
                        for (let [key, value] of Object.entries(summary.summary)) {
                            yPosition = addText(`• ${key}: ${typeof value === 'object' ? JSON.stringify(value) : value}`, margin + 5, yPosition, pageWidth - 2 * margin - 5, 10)
                            yPosition += 2
                        }
                    }
                    yPosition += 10
                ...
                if (summary.medications) {
                    yPosition = addText('CURRENT MEDICATIONS', margin, yPosition, pageWidth - 2 * margin, 14, 'bold')
                    yPosition += 5

                    if (typeof summary.medications === 'string') {
                        const medicationLines = summary.medications.split(/\n|;|\*|•|-/).filter(line => line.trim())
                        let medCount = 1
                        for (let line of medicationLines) {
                            const cleanLine = line.trim().replace(/^[-•*]\s*/, '')
                            if (cleanLine) {
                                yPosition = addText(`${medCount}. ${cleanLine}`, margin + 5, yPosition, pageWidth - 2 * margin - 5, 10)
                                yPosition += 2
                                medCount++
                            }
                        }
                        ...
                        let medCount = 1
                        for (let med of summary.medications) {
                            if (typeof med === 'object' && med !== null) {
                                let medText = `${medCount}. ${med.name || 'Medication'}`
                                if (med.dosage) medText += ` - ${med.dosage}`
                                if (med.frequency) medText += ` (${med.frequency})`
                                if (med.duration) medText += ` for ${med.duration}`
                                yPosition = addText(medText, margin + 5, yPosition, pageWidth - 2 * margin - 5, 10)
                                if (med.instructions) {
                                    yPosition = addText(`   Instructions: ${med.instructions}`, margin + 10, yPosition, pageWidth - 2 * margin - 10, 9, 'italic')
                                }
                            } else {
                                yPosition = addText(`${medCount}. ${med}`, margin + 5, yPosition, pageWidth - 2 * margin - 5, 10)
                            }
                            yPosition += 2
                            medCount++
                        }
                        ...
                        let medCount = 1
                        for (let [key, value] of Object.entries(summary.medications)) {
                            yPosition = addText(`${medCount}. ${key}: ${typeof value === 'object' ? JSON.stringify(value) : value}`, margin + 5, yPosition, pageWidth - 2 * margin - 5, 10)
                            yPosition += 2
                            medCount++
                        }
                    }
                ...
                if (summary.insights) {
                    yPosition = addText('CLINICAL INSIGHTS', margin, yPosition, pageWidth - 2 * margin, 14, 'bold')
                    yPosition += 5
                    
                    if (typeof summary.insights === 'string') {
                        const insightLines = summary.insights.split('\n').filter(line => line.trim())
                        for (let line of insightLines) {
                            if (line.trim()) {
                                yPosition = addText(`• ${line.trim()}`, margin + 5, yPosition, pageWidth - 2 * margin - 5, 10)
                                yPosition += 2
                            }
                        }
                    } else if (Array.isArray(summary.insights)) {
                        for (let insight of summary.insights) {
                            yPosition = addText(`• ${typeof insight === 'object' ? JSON.stringify(insight) : insight}`, margin + 5, yPosition, pageWidth - 2 * margin - 5, 10)
                            yPosition += 2
                        }
                    } else if (typeof summary.insights === 'object') {
                        for (let [key, value] of Object.entries(summary.insights)) {
                            yPosition = addText(`• ${key}: ${typeof value === 'object' ? JSON.stringify(value) : value}`, margin + 5, yPosition, pageWidth - 2 * margin - 5, 10)
                            yPosition += 2
                        }
                    }
                ...
                if (summary.recommendations) {
                    yPosition = addText('RECOMMENDATIONS', margin, yPosition, pageWidth - 2 * margin, 14, 'bold')
                    yPosition += 5
                    
                    if (typeof summary.recommendations === 'string') {
                        const recommendationLines = summary.recommendations.split('\n').filter(line => line.trim())
                        for (let line of recommendationLines) {
                            if (line.trim()) {
                                yPosition = addText(`• ${line.trim()}`, margin + 5, yPosition, pageWidth - 2 * margin - 5, 10)
                                yPosition += 2
                            }
                        }
                    } else if (Array.isArray(summary.recommendations)) {
                        for (let rec of summary.recommendations) {
                            yPosition = addText(`• ${typeof rec === 'object' ? JSON.stringify(rec) : rec}`, margin + 5, yPosition, pageWidth - 2 * margin - 5, 10)
                            yPosition += 2
                        }
                    } else if (typeof summary.recommendations === 'object') {
                        for (let [key, value] of Object.entries(summary.recommendations)) {
                            yPosition = addText(`• ${key}: ${typeof value === 'object' ? JSON.stringify(value) : value}`, margin + 5, yPosition, pageWidth - 2 * margin - 5, 10)
                            yPosition += 2
                        }
                    }
            ...
            pdf.text('This report was generated by Ayurveda Healthcare System. For medical emergencies, please contact your healthcare provider immediately.', margin, footerY, { maxWidth: pageWidth - 2 * margin })

            // Save the PDF
            const fileName = `patient-summary-${patientData.name.replace(/\s+/g, '-')}-${new Date().toISOString().split('T')[0]}.pdf`
            pdf.save(fileName)
                        ...
                        <div className='p-6 border-t border-gray-200 bg-gray-50'>
                            <div className='flex justify-end space-x-3'>
                                <button 
                                    ...
                                    className='inline-flex items-center px-4 py-2 border border-green-300 rounded-lg text-green-700 bg-green-50 hover:bg-green-100 hover:border-green-400 transition-colors'
                                >
