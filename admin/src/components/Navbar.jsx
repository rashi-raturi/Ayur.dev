...
const Navbar = () => {
  ...
  const getUserName = () => {
    ...
    return aToken ? 'Admin User' : 'Doctor'
  }
  ...
  const getUserRole = () => {
    ...
    return aToken ? 'Administrator' : 'Ayurvedic Physician'
  }
  ...
  return (
    <div className={`flex justify-between items-center py-4 bg-white border-b border-gray-200 transition-all duration-300 ${isSidebarOpen ? 'ml-64 px-6' : 'ml-0 pl-20 pr-6'}`}>
      {/* Left Side - Page Title */}
      <div>
        <h1 className='text-xl md:text-2xl font-semibold text-gray-900'>{getPageTitle()}</h1>
        <p className='text-xs md:text-sm text-gray-500 mt-1 hidden sm:block'>Overview of your practice and patient statistics</p>
      </div>
      ...
      <div className='flex items-center gap-3 md:gap-6'>
        ...
        >
          <div className='text-right hidden md:block'>
            <p className='text-sm font-semibold text-gray-900 capitalize'>{getUserName()}</p>
